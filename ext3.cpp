...
// double strom_th=175e-6; // for L_in

double strom_th=125e-6; // for L_out
...
//---------------------------------------------------	   
	   ...
	   werteint[n]=0;     // initialer Zustand 0
	   n++;
	}      
	   ...
	   if (i==n)            // eine Spalte voll, jetzt Auswertung
	     {
	         {       
		   x=werte1[1];         // Spulenstrom
		   if (x<strom_th) y=0; else y=1;
                  		   	   
		   if (y!=werteint[i])  // Zustand der Speicherschleife
		     {
  //		        printf("..%d, %lf %lf %lf %lf \n",i,werte0[0], werte1[0], werte0[1], werte1[1]);
		     
		        zeit_a=werte0[0];
			zeit_b=werte1[0];
			phase_a=werte0[i-1];
			phase_b=werte1[i-1];
		        dt=(strom_th-phase_a)*(zeit_b-zeit_a)/(phase_b-phase_a);
...
//------------------------------
			bits[pclock]=-1;			
                        if (scount>0)
			  {
			    if ( (junction[scount]==0) && (junction[scount-1]==1) )			    
			      {  // strom geht aus
               			if (i==1) pclock++;			      
			    ...
			    if ( (junction[scount]==1) && (junction[scount-1]==0)  )
			      {  // Strom geht an
			        pone++;
				// nichts
			      }
...
//------------------------------	                                     
    ...
    for (i=1; i<scount; i++)
      if ( (junction[i]==0) && (junction[i-1]==1) )      
        {
  ...
  //         printf("# %lf\n",t);
    ...
    printf(    "%d %d %lf %lf \n", pclock, pone, timesum, streusum );   
    fprintf(ey,"%d %d %lf %lf \n", pclock, pone, timesum, streusum );
